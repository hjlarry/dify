...
const matchesSearchQuery = (
  ...
  // Match plugin_id
  if (plugin.plugin_id.toLowerCase().includes(lowerQuery)) return true
  // Match plugin name
  if (plugin.name?.toLowerCase().includes(lowerQuery)) return true
  // Match declaration name
  if (declaration.name?.toLowerCase().includes(lowerQuery)) return true
...
const PluginsPanel = ({
  ...
  const categoryList = useMemo(() => {
    if (!fixedCategory) return pluginListWithLatestVersion

    return pluginListWithLatestVersion.filter(
      (plugin) => plugin.declaration.category === fixedCategory,
    )
  ...
  const builtinTools = isToolIntegrationPage
    ? (pluginList?.builtin_tools ?? EMPTY_BUILTIN_TOOLS)
    : EMPTY_BUILTIN_TOOLS
  const filteredBuiltinTools = useMemo(() => {
    if (!isToolIntegrationPage) return []
  ...
  const isFilteringCategory =
    !!filters.searchQuery.trim() || (supportsTagFilter && filters.tags.length > 0)
  const isIntegrationCategorySearchEmpty =
  ...
  const toolbar = (
    ...
    >
      {!layout && !isIntegrationCategoryPage && (
